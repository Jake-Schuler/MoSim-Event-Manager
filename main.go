...
func main() {
	...
	r.GET("/register", func(c *gin.Context) {
		...
		if access_token == "" {
			c.HTML(400, "authRedirect.tmpl", gin.H{"error": "access_token is required", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}

		req, err := http.NewRequest("GET", "https://discord.com/api/users/@me", nil)
		if err != nil {
			c.HTML(500, "authRedirect.tmpl", gin.H{"error": "Failed to create request", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}
		...
		if err != nil {
			c.HTML(500, "authRedirect.tmpl", gin.H{"error": "Failed to contact Discord", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}
		defer resp.Body.Close()

		if resp.StatusCode != 200 {
			c.HTML(400, "authRedirect.tmpl", gin.H{"error": "Invalid access token", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}

		var userInfo map[string]interface{}
		if err := json.NewDecoder(resp.Body).Decode(&userInfo); err != nil {
			c.HTML(500, "authRedirect.tmpl", gin.H{"error": "Failed to parse Discord response", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}

		idStr, ok := userInfo["id"].(string)
		if !ok {
			c.HTML(500, "authRedirect.tmpl", gin.H{"error": "Discord response missing id", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}
		idInt, err := strconv.Atoi(idStr)
		if err != nil {
			c.HTML(500, "authRedirect.tmpl", gin.H{"error": "Invalid Discord user id", "ClientID": os.Getenv("DISCORD_CLIENT_ID"), "RedirectURI": os.Getenv("DISCORD_REDIRECT_URI")})
			return
		}
	...
	authorized.GET("/generate", func(c *gin.Context) {
		...
		switch runtime.GOOS {
		case "windows":
			matchMakerExePath = filepath.Join(cwd, "data/MatchMaker.exe")
		case "linux":
			matchMakerExePath = filepath.Join(cwd, "data/MatchMaker")
		}
